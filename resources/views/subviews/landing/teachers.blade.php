<section class="container-fluid pt-4 px-0 relative" id="teachers-section">
  <!-- Title -->
  <h2 class="my-subtitle-letter my-section-subtitle text-center" id="teachers-subtitle">CONOCE A NUESTROS</h2>
    ...
    @foreach ($viewData['teachers'] as $teacher)
      <div class="item">
        <div class="teacher-container text-center">
          <img src="{{ asset('storage/teachers/' . $teacher->getImage()) }}" class="image-circle"
            alt="{{ $teacher->getName() }}">
          <p class="teacher-name text-dark mt-4 my-subtitle-letter my-section-subtitle teacher-name-spacing">
            {{ $teacher->getName() }}</p>
        </div>
      </div>
    @endforeach
